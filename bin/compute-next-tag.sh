...
#
# Tags look like `v<Jackett version>-<release>`, which is what this repository
# already publishes (`v0.24.1870-3`):
#
# - if defaults/main.yml points at a Jackett version that has never been
#   released, the release counter restarts at 0 (`v0.24.2467-0`)
# - otherwise the counter is incremented (`v0.24.1870-4`), but only if
...
# The commit-message approach this replaced never fired once: it looked through
# the last 20 commits for a renovate[bot] commit whose subject contained
# "docker tag to", and Renovate had never been able to propose a bump for this
# role at all - it was looking `linuxserver/jackett` up on a registry host whose
# tag listing it gives up on long before reaching the recent tags. Every version
# bump this repository has ever seen was typed in by hand.
...
# Anchored on `jackett_version:` so that neither a commented-out example nor
# `jackett_container_image_tag`, which is derived from it, can be mistaken for
# it.
version="$(sed -nE 's|^jackett_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Jackett version from $defaults_path"
	exit 1
fi

# The value carries no leading `v` while the tags do. It used to carry one
# (the `v0.24.766-0` tag was cut from a `jackett_version: v0.24.766`), so
# tolerate it rather than producing a doubled prefix if it ever comes back.
...
if [ -z "$last_release" ]; then
	echo >&2 "Version ${version#v} has never been released"
	echo "${tag_prefix}0"
	exit 0
fi
